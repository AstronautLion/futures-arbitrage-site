...
const ASSET_VERSION = '20260405-1';
...
function normalizeRefreshApiBaseUrl(value) {
  ...
  // If starts with // (protocol-relative), prefix with http:
...
function canUseImplicitRefreshAuth() {
  return Boolean(hasRefreshApi() && isSameOriginRefreshApi() && state.refreshApiHealth?.refresh_auth_cookie_enabled);
}
...
const TRANSLATIONS = {
  en: {
    ...
    'button.openGlossary': 'Metric glossary',
    ...
    'guide.watch.body': 'After the first pass, keep only rows where net spread is above your threshold, liquidity is not thin and exchange health is clean. If the idea depends only on a pretty raw spread, leave it aside until the structure becomes cleaner.',
    ...
    'panel.glossary.title': 'Glossary',
    'panel.glossary.desc': 'Definitions, formulas, interpretation ranges and practical actions for key dashboard and AI metrics.',
    'panel.glossary.search': 'Search term',
    'panel.glossary.searchPlaceholder': 'confidence, funding, whale...',
  ...
  ru: {
    ...
    'button.openGlossary': 'Справочник метрик',
    ...
    'guide.read.body': 'Смотрите не просто на широкий спред, а на рабочее расхождение: после комиссий у него должно оставаться место для движения обратно к нулю. Если к выходу чистый спред заметно приблизился к нулю, идея действительно отработала.',
    'guide.watch.title': 'На что смотреть',
    'guide.watch.body': 'После первого просмотра оставляйте в работе только пары, где чистый спред выше вашего порога, ликвидность достаточная, а по биржам нет явной деградации. Если идея держится только на красивом исходном спреде, лучше перевести её в наблюдение.',
    'guide.use.title': 'Как использовать сигналы',
    'guide.use.body': 'Порядок действий такой: сначала соберите короткий список по чистому спреду и ликвидности, затем откройте карточку пары и проверьте исполнение и ограничения риска, и только потом переходите в ИИ-аналитику за таймингом входа, ожидаемой доходностью, оценкой времени до схождения и подсказкой по маршруту исполнения.',
    ...
    'panel.glossary.desc': 'Определения, формулы, шкалы интерпретации и практические действия по ключевым метрикам дашборда и AI-аналитики.',
...
const state = {
  ...
  datasetStatus: {
    ...
    glossary: { loaded: false, loading: false, error: '' },
  },
  datasetPromises: {},
  historyIndex: [],
...
function loadSettings() {
  try {
    ...
    return {
      ...
      visibleCrossColumns: parsed.visibleCrossColumns && typeof parsed.visibleCrossColumns === 'object' && !Array.isArray(parsed.visibleCrossColumns) && Object.keys(parsed.visibleCrossColumns).length
  ...
  const accessBadge = !hasRefreshApi()
    ...
    : state.refreshApiProbeError === 'unreachable'
      ...
      : state.refreshApiHealth?.refresh_auth_required
        ? {
            tone: (getRefreshApiKey() || canUseImplicitRefreshAuth()) ? 'positive' : 'warning',
            text: (getRefreshApiKey() || canUseImplicitRefreshAuth()) ? t('refresh.accessReady') : t('refresh.accessMissing'),
          }
        : {
            tone: 'positive',
...
async function fetchOpportunityAiBundle(item) {
  ...
  const [confidence, predictions, whale, liquidation, smartRoute, executionPayload, advicePayload] = await Promise.all([
    ...
    fetchAiJson(AI_EXECUTION_ENDPOINT, {
      method: 'POST',
      body: JSON.stringify({ exchangeA: item.exchangeA, exchangeB: item.exchangeB, symbol: item.pairA }),
    }),
    fetchAiJson(AI_ADVICE_ENDPOINT, {
      method: 'POST',
      body: JSON.stringify({ exchangeA: item.exchangeA, exchangeB: item.exchangeB, symbol: item.pairA }),
    }),
  ]);
...
function buildCrossOpportunity(item) {
  ...
  const finalItem = {
    ...
    aiAdvice: aiAdvice,
    aiAdviceSummary: aiAdvice.summary || '',
    aiAdviceVerdict: aiAdvice.verdict || '',
    aiAdviceEntryFocus: Array.isArray(aiAdvice.entry_focus) ? aiAdvice.entry_focus : [],
    aiAdviceHoldingFocus: Array.isArray(aiAdvice.holding_focus) ? aiAdvice.holding_focus : [],
    aiAdviceExitFocus: Array.isArray(aiAdvice.exit_focus) ? aiAdvice.exit_focus : [],
    aiAdviceRiskWatch: Array.isArray(aiAdvice.risk_watch) ? aiAdvice.risk_watch : [],
    aiAdviceNextActions: Array.isArray(aiAdvice.next_actions) ? aiAdvice.next_actions : [],
    aiSnapshot: ai,
    entryWaitMinutes: numberOrNull(aiEntry.wait_minutes),
    entryConfidence: numberOrNull(aiEntry.confidence),
    expectedEntrySpreadPct: numberOrNull(aiEntry.expected_spread_pct),
    exitEtaMinutes: numberOrNull(aiExit.eta_minutes),
    exitTargetSpreadPct: numberOrNull(aiExit.target_spread_pct),
    exitType: aiExit.exit_type || null,
    holdingHoursRecommended: numberOrNull(aiHolding.recommended_hours),
...
function computeExecutionReality(item, derived, executionDifficulty, marketPermission, healthQuality) {
  ...
  const safeParticipation = (maxBidAsk <= 0.05 ? 0.0035 : maxBidAsk <= 0.1 ? 0.002 : 0.0008) * healthMultiplier * permissionMultiplier;
  const safeSizeUsd = Math.round(clamp(minVolume * safeParticipation, 100, Math.max(250, minVolume * 0.02 || 250)));
  const positionPressure = notional / Math.max(1, safeSizeUsd);
  const expectedSlippagePercent = Number(clamp(maxBidAsk * 0.58 + positionPressure * 0.16 + executionDifficulty * 0.006, 0.02, 4.5).toFixed(3));
  ...
  const score = Math.round(clamp(
    100 - expectedSlippagePercent * 24 - executionDifficulty * 0.34 + (healthQuality - 50) * 0.15 + (marketPermission.mode === 'open' ? 6 : marketPermission.mode === 'cautious' ? -4 : -12),
    5,
    100,
  ));
  return {
    ...
    netAfterExecution,
    effectiveProfit,
    requiresScaling: notional > safeSizeUsd,
  };
}

function computeOverlapStats(item) {
  const pairRows = state.cross.filter((row) => String(row.pairA || row.pair || '') === String(item.pairA || item.pair || ''));
...
function getFilteredCross() {
  ...
  const value = rows
    ...
    .filter((item) => (maxAiEntryWaitMinutes > 0 && item.entryWaitMinutes != null ? item.entryWaitMinutes <= maxAiEntryWaitMinutes : true))
...
function renderCrossCell(column, item) {
  ...
  if (column.key === 'entryWaitMinutes') {
    return buildCell(
      column.label,
      item.entryWaitMinutes == null ? pickLocalized('n/a', 'н/д') : formatEtaMinutes(item.entryWaitMinutes),
      item.entryWaitMinutes != null && item.entryWaitMinutes <= 5 ? 'value-positive' : item.entryWaitMinutes != null && item.entryWaitMinutes <= 20 ? 'value-warning' : '',
    );
  }
...
function renderAiDetailCards(item) {
  ...
  return `
    <section class="detail-note-card risk-filter-card">
      <h3>${escapeHtml(pickLocalized('Qwen trade advice', 'Совет Qwen по сделке'))}</h3>
      <p class="detail-note-copy"><span class="chip ${escapeHtml(adviceChipClass)}">${escapeHtml(adviceVerdictLabel)}</span> ${escapeHtml(item.aiAdviceSummary || pickLocalized('Detailed neural advice is not available yet for this pair.', 'Подробный нейросовет по этой паре пока недоступен.'))}</p>
      ${adviceSections.length ? adviceSections.map((section) => `
...
function renderHealthPage() {
  ...
  const alertSummary = state.metrics?.alert_summary || {};
...
function renderAiAnalyticsPage() {
  ...
  dom.aiSummary.innerHTML = [
    ...
    },
    {
      ...
      detail: fastestRow.entryWaitMinutes != null
        ? `${translateAiDecision(fastestRow.decision)} · ${pickLocalized('entry', 'вход')} ${formatEtaMinutes(fastestRow.entryWaitMinutes)}`
      ...
      <article class="ai-analytics-card ${item.whaleIndex >= 70 || item.liquidationRiskPct >= 18 ? 'is-alert' : ''}">
        ...
        <div class="ai-analytics-grid">
          ...
          <div class="ai-analytics-metric">
            ...
            <strong>${escapeHtml(item.entryWaitMinutes == null ? pickLocalized('n/a', 'н/д') : formatEtaMinutes(item.entryWaitMinutes))}</strong>
          </div>
          <div class="ai-analytics-metric">
...
function renderHealthDiagnostics() {
  ...
  const alerts = Array.isArray(state.metrics?.alerts) ? state.metrics.alerts : [];
  const alertSummary = state.metrics?.alert_summary || {};
  const adaptivePublication = state.metrics?.analytics_summary?.adaptive_publication || {};
  const quoteAge = state.metrics?.analytics_summary?.quote_age || {};
    ...
    <article class="detail-note-card health-diagnostic-card detail-action-card">
      ...
      ${alerts.length ? `
        <ul class="inline-note-list">
          ${alerts.slice(0, 4).map((alert) => `<li>${escapeHtml(`${translateAlertSeverity(alert.severity || 'info')}: ${String(alert.message || alert.reason || pickLocalized('backend alert', 'backend alert'))}`)}</li>`).join('')}
        </ul>
      ` : ''}
    </article>
