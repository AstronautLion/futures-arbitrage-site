...
const AUTO_REFRESH_DEFAULT_INTERVAL_SECONDS = 30;
...
const AUTO_REFRESH_DEFAULT_FULL_INTERVAL_SECONDS = 600;
...
const ENTRY_READY_MAX_WAIT_MINUTES = 15;
...
const LIVE_DATA_RENDER_INTERVAL_MS = 10000;
const AI_SNAPSHOT_STALE_MS = 180_000;
const MARKET_STREAM_RECONNECT_BASE_MS = 500;
const MARKET_STREAM_RECONNECT_MAX_MS = 5000;
const MARKET_STREAM_STARTUP_WAIT_MS = 1500;
const MARKET_STREAM_FRESHNESS_MS = 15000;
const SWR_DEDUP_INTERVAL_MS = 2000;
const SWR_FOCUS_THROTTLE_MS = 5000;
const SWR_RECONNECT_THROTTLE_MS = 3000;
const FUNDING_COUNTDOWN_TICK_MS = 2000;
const PIN_BAN_STORAGE_KEY = 'fas-pin-ban-v1';
...
// ---------------------------------------------------------------------------
// Exchange trade URL builders (for direct links to exchange trading pages)
// ---------------------------------------------------------------------------
const EXCHANGE_TRADE_URLS = {
  bybit: (pair) => `https://www.bybit.com/trade/usdt/${pair}`,
  okx: (pair) => `https://www.okx.com/trade-swap/${pair.toLowerCase()}`,
  kucoin: (pair) => `https://www.kucoin.com/futures/trade/${pair}`,
  mexc: (pair) => `https://futures.mexc.com/exchange/${pair}`,
  gateio: (pair) => `https://www.gate.io/futures_trade/USDT/${pair}`,
  bitget: (pair) => `https://www.bitget.com/futures/usdt/${pair}`,
  bingx: (pair) => `https://www.bingx.com/en-us/futures/${pair}`,
  phemex: (pair) => `https://phemex.com/trade/${pair}`,
  kraken: (pair) => `https://futures.kraken.com/trade/${pair}`,
  whitebit: (pair) => `https://whitebit.com/futures/trade/${pair}`,
  bitmart: (pair) => `https://futures.bitmart.com/en/trade/${pair}`,
  lbank: (pair) => `https://www.lbank.com/futures/${pair}`,
  htx: (pair) => `https://www.htx.com/futures/exchange/${pair}`,
  deribit: (pair) => `https://www.deribit.com/trade/${pair}`,
  coincatch: (pair) => `https://www.coincatch.com/futures/usdt/${pair}`,
  deepcoin: (pair) => `https://www.deepcoin.com/en/futures/${pair}`,
  bitunix: (pair) => `https://www.bitunix.com/futures/${pair}`,
  tapbit: (pair) => `https://www.tapbit.com/futures/${pair}`,
  pionex: (pair) => `https://www.pionex.com/en/futures/${pair}`,
  blofin: (pair) => `https://www.blofin.com/futures/${pair}`,
  apex: (pair) => `https://www.apex.exchange/trade/${pair}`,
  weex: (pair) => `https://www.weex.com/futures/${pair}`,
  zoomex: (pair) => `https://www.zoomex.com/futures/${pair}`,
  ourbit: (pair) => `https://www.ourbit.com/futures/${pair}`,
  btcc: (pair) => `https://www.btcc.com/futures/${pair}`,
  coinw: (pair) => `https://www.coinw.com/futures/${pair}`,
};

function buildExchangeTradeUrl(exchange, pair) {
  const normalizedExchange = String(exchange || '').toLowerCase().replace(/[\s_-]/g, '');
  const builder = EXCHANGE_TRADE_URLS[normalizedExchange];
...
// Pin / Ban persistence
// ---------------------------------------------------------------------------
function loadPinBanState() {
  try {
    const raw = localStorage.getItem(PIN_BAN_STORAGE_KEY);
    if (!raw) return { pinnedIds: [], bannedTokens: [] };
    const parsed = JSON.parse(raw);
    return {
      pinnedIds: Array.isArray(parsed.pinnedIds) ? parsed.pinnedIds : [],
      bannedTokens: Array.isArray(parsed.bannedTokens) ? parsed.bannedTokens : [],
    };
  } catch {
    return { pinnedIds: [], bannedTokens: [] };
  }
}

function savePinBanState() {
  try {
    localStorage.setItem(PIN_BAN_STORAGE_KEY, JSON.stringify({
...
function toggleBanToken(token) {
  const normalized = String(token || '').toUpperCase();
  if (state.bannedTokens.has(normalized)) {
    state.bannedTokens.delete(normalized);
  } else {
    state.bannedTokens.add(normalized);
  }
  savePinBanState();
  renderAll();
}
...
function scheduleAutoRefresh(delayOverrideMs = null) {
  ...
  state.autoRefreshTimer = window.setTimeout(async () => {
    ...
    // Always try to refresh — don't skip based on SWR freshness
    if (state.refreshInFlight || state.refreshState?.status === 'running') {
      scheduleAutoRefresh(getAutoRefreshBusyRetryDelayMs());
      return;
    }
    // If data is stale (> 3 min), force a full reload from disk as safety net
    const isStale = state.lastRefreshAt && (Date.now() - new Date(state.lastRefreshAt).getTime() > 180000);
    if (isStale && !hasFreshMarketStreamData()) {
      try {
        await loadAllData(true);
        setRefreshStatus(t('refresh.snapshotReloaded'));
      } catch {
        // fall through to handleRefreshButton
      }
    }
    try {
...
function getSystemAlertSummary() {
  // Always prefer metrics alert_summary (most authoritative source)
...
function getSystemAlertsList() {
  // Always prefer metrics alerts (most authoritative source)
...
const state = {
  ...
  pinnedIds: new Set(loadPinBanState().pinnedIds),
  bannedTokens: new Set(loadPinBanState().bannedTokens),
...
function bindEvents() {
  ...
  dom.crossTable?.addEventListener('click', (event) => {
    ...
    if (pinBtn) {
      ...
      togglePinId(pinBtn.dataset.pinId);
      return;
    }
    // Ban button handler
    const banBtn = event.target.closest('[data-ban-token]');
    if (banBtn) {
      event.stopPropagation();
      toggleBanToken(banBtn.dataset.banToken);
      return;
    }
  ...
  dom.tradeSmartNotices?.addEventListener('click', (event) => {
    const btn = event.target.closest('[data-trade-notice-open]');
    if (!btn) return;
    const item = getFilteredCross().find((entry) => entry.id === btn.dataset.tradeNoticeOpen);
    if (item) openOpportunityDetail(item);
  });
...
function refreshVisibleTradeAlertUi() {
  // Batch schedule the update instead of doing synchronous DOM queries
  batchScheduleLiveUpdate();
...
function renderTradeSmartNotices() {
  ...
  dom.tradeSmartNotices.innerHTML = notices
    ...
    `)
    .join('');
  // Use delegated event listener instead of attaching to every button
}
...
function hydrateVisibleCrossAi(rows) {
  ...
  // Debounce: only fetch for first visible row to reduce API load
  const [firstRow] = rows.slice(0, 1);
  if (!firstRow) return;
  
  const snapshot = getAiSnapshot(firstRow.id);
  const horizon = normalizeAiPredictionHorizon(state.settings.aiPredictionHorizon);
  if (snapshot?.loading) {
    return;
  }
  ...
  if (snapshot?.complete && snapshot?.requestedHorizon === horizon && !isAiSnapshotStale(snapshot, firstRow)) {
    return;
  }
  void ensureOpportunityAiData(firstRow);
}
...
function batchScheduleLiveUpdate() {
  if (state.liveUpdateScheduled) return;
  state.liveUpdateScheduled = true;
  window.requestAnimationFrame(() => {
    refreshCrossLiveTimingCells();
    state.liveUpdateScheduled = false;
  });
}
...
function refreshCrossLiveTimingCells() {
  ...
  rows.forEach((row) => {
    ...
    const item = itemMap.get(detailId);
    if (!item) return;
    ...
    if (entryCell || tradeAlertCell || positionCell) {
      updates.push({ row, entryCell, tradeAlertCell, positionCell, item, liveEntryWaitMinutes, tradeAlertModel });
    }
  });
  
  // Batch all writes
  updates.forEach(({ entryCell, tradeAlertCell, positionCell, liveEntryWaitMinutes, tradeAlertModel }) => {
    if (entryCell) {
      ...
      const newClass = liveEntryWaitMinutes != null && liveEntryWaitMinutes <= 5
        ? 'value-positive'
        : liveEntryWaitMinutes != null && liveEntryWaitMinutes <= 20
          ? 'value-warning'
          : '';
      if (entryCell.className !== newClass) {
        entryCell.className = newClass;
      }
    }
...
function runLiveDataTick() {
  ...
  renderFreshnessBadge();
  // Auto-recover: if data is stale and market stream is not connected, force refresh
  if (state.lastRefreshAt && (Date.now() - new Date(state.lastRefreshAt).getTime() > 300000) && !hasFreshMarketStreamData() && !state.refreshInFlight) {
    void handleRefreshButton('filtered', { trigger: 'auto' });
  }
  if (state.activeTab === 'cross' || state.activeTab === 'ai' || state.activeDetailId) {
    refreshLiveDataForActiveTab();
  }
...
function buildFundingCountdownHtml(nextFundingTime, intervalHours) {
  if (!nextFundingTime) return '';
  // Defer time calculation to update function - render placeholder
  const intervalLabel = intervalHours ? ` ${intervalHours}h` : '';
  return `<span class="funding-countdown" data-funding-countdown="${escapeHtml(nextFundingTime)}">--:--:--${intervalLabel}</span>`;
}

function updateFundingCountdowns() {
  const elements = document.querySelectorAll('[data-funding-countdown]');
  if (!elements.length) return;
  
  const now = Date.now();
  elements.forEach((el) => {
    const targetTime = el.getAttribute('data-funding-countdown');
    if (!targetTime) return;
    const ms = new Date(targetTime).getTime() - now;
    if (ms <= 0) {
      el.textContent = 'LIVE';
      el.className = 'funding-countdown funding-countdown-live';
      return;
    }
    ...
    const s = totalSec % 60;
    const urgency = totalSec < 300 ? 'funding-countdown-urgent' : totalSec < 900 ? 'funding-countdown-warning' : '';
    const existingText = el.textContent || '';
    const intervalLabel = existingText.match(/\s\d+h$/) ? existingText.match(/\s\d+h$/)[0] : '';
    el.textContent = `${h}:${String(m).padStart(2, '0')}:${String(s).padStart(2, '0')}${intervalLabel}`;
    el.className = `funding-countdown ${urgency}`;
  });
}
...
// OPTIMIZATION: Pre-compile scoring functions to reduce per-item overhead
const _scoringFnCache = {};
function _getCachedScoringFn(name) {
  if (!_scoringFnCache[name]) {
    // Memoize small, frequently called functions
    if (name === 'spreadQuality') {
      _scoringFnCache[name] = (v) => scoreSpreadQuality(v);
    }
  }
  return _scoringFnCache[name];
}

function getCrossOpportunityRows() {
  // OPTIMIZATION: Batch check cache validity before mapping
  const invalidated = state.crossOpportunityItemCache.invalidated || false;
  const result = state.cross.map((item) => {
    // Early return from cache if item not changed
    if (!invalidated) {
  ...
  // Reset invalidation flag after bulk operation
  state.crossOpportunityItemCache.invalidated = false;
  return result;
}
...
function buildFilteredCrossSignature() {
  ...
  // OPTIMIZATION: Cache visible exchanges string instead of recomputing
  const visibleExchangesKey = state.visibleExchangesKey || (
    state.visibleExchangesKey = getAllExchanges()
      .map((exchange) => `${exchange}:${state.settings.visibleExchanges[exchange] !== false ? 1 : 0}`)
      .join('|')
  );
  return `${state.crossOpportunityRevision}__${query}__${minSpread}__${minAiScore}__${maxAiEntryWaitMinutes}__${maxAiHoldingHours}__${visibleExchangesKey}`;
}
...
function getSortedCrossRows() {
  ...
  // OPTIMIZATION: Use pinned IDs size as cache key component
  const pinnedKey = state.pinnedIds.size > 0 ? [...state.pinnedIds].sort().join(',') : '';
  const signature = `${state.filteredCrossCache.signature}__${sort.key || ''}__${sort.direction || 'none'}__${pinnedKey}`;
  
  if (state.sortedCrossCache.signature === signature) {
    return state.sortedCrossCache.value;
  }
  
  const sorted = sort.key && sort.direction !== 'none'
    ? sortRows(rows, 'cross')
  ...
  // OPTIMIZATION: Single pass for pinning instead of filter + filter
  const pinned = [];
  const unpinned = [];
  for (let i = 0; i < sorted.length; i++) {
    if (state.pinnedIds.has(sorted[i].id)) {
      pinned.push(sorted[i]);
    } else {
      unpinned.push(sorted[i]);
    }
  }
  
  const value = pinned.length > 0 ? [...pinned, ...unpinned] : sorted;
  state.sortedCrossCache = { signature, value };
  return value;
}
...
function getFilteredCross() {
  ...
  // OPTIMIZATION: Single pass filter instead of 7 chained filters
  const value = [];
  const bannedTokens = state.bannedTokens;
  const hasQuery = query.length > 0;
  const hasMinAiScore = minAiScore > 0;
  const hasEntryWait = maxAiEntryWaitMinutes > 0;
  const hasHoldingHours = maxAiHoldingHours > 0;
  
  for (let i = 0; i < rows.length; i++) {
    const item = rows[i];
    if (bannedTokens.has(String(item.pairA || '').toUpperCase())) continue;
    if (!isExchangeVisible(item.exchangeA) || !isExchangeVisible(item.exchangeB)) continue;
    if (hasQuery && !`${item.pairA}${item.exchangeA}${item.exchangeB}`.toUpperCase().includes(query)) continue;
    if (item.netSpread < minSpread) continue;
    if (hasMinAiScore && item.aiScore != null && item.aiScore < minAiScore) continue;
    if (hasEntryWait) {
      const liveEntryWaitMinutes = getLiveEntryWaitMinutes(item);
      if (liveEntryWaitMinutes != null && liveEntryWaitMinutes > maxAiEntryWaitMinutes) continue;
    }
    if (hasHoldingHours && item.holdingHoursRecommended != null && item.holdingHoursRecommended > maxAiHoldingHours) continue;
    value.push(item);
  }
  state.filteredCrossCache = { signature, value };
  return value;
...
function getFilteredFunding() {
  ...
  // OPTIMIZATION: Single pass filter
  const result = [];
  for (let i = 0; i < funding.length; i++) {
    const item = funding[i];
    if (!isExchangeVisible(item.exchangeA) || !isExchangeVisible(item.exchangeB)) continue;
    if (hasQuery && !`${item.pair}${item.exchangeA}${item.exchangeB}`.toUpperCase().includes(query)) continue;
    result.push(item);
  }
  return result;
}
...
function renderCrossTable() {
  ...
  const beforeSpacer = renderVirtualSpacer(columnCount, isDesktopVirtualized ? virtualWindow.beforeHeight : 0);
  const afterSpacer = renderVirtualSpacer(columnCount, isDesktopVirtualized ? virtualWindow.afterHeight : 0);
  
  // OPTIMIZATION: Pre-compute trade alert models for visible rows in batch
  const tradeAlertCache = {};
  for (let i = 0; i < virtualWindow.rows.length; i++) {
    const item = virtualWindow.rows[i];
    tradeAlertCache[item.id] = getTradeAlertModel(item);
  }
  
  const rowsHtml = virtualWindow.rows.map((item) => {
    const pinnedClass = pinnedIds.has(item.id) ? ' is-pinned' : '';
    const cells = visibleColumns.map((column) => renderCrossCell(column, item, tradeAlertCache[item.id])).join('');
    return `<tr class="opportunity-row${pinnedClass }" tabindex="0" data-detail-id="${item.id}">${cells}</tr>`;
  }).join('');
  dom.crossTable.querySelector('tbody').innerHTML = `${beforeSpacer}${rowsHtml}${afterSpacer}`;
...
function renderCrossMobileCard(item) {
  ...
  const tradeAlertModel = getTradeAlertModel(item);
  const reliabilityChip = renderReliabilityChip(item);
  const directionEscaped = escapeHtml(item.direction);
  const lifecycleEscaped = escapeHtml(item.lifecycle);
  const regimeEscaped = escapeHtml(item.regime);
  const regimeLabelEscaped = escapeHtml(item.regimeLabel);
  const pairAEscaped = escapeHtml(item.pairA);
  const exchangeAEscaped = escapeHtml(item.exchangeA);
  const exchangeBEscaped = escapeHtml(item.exchangeB);
  const idEscaped = escapeHtml(item.id);
  return `
    <button class="cross-mobile-card card-enter" type="button" data-detail-id="${idEscaped}">
      <div class="cross-mobile-topline">
        <div>
          <strong class="cross-mobile-pair">${pairAEscaped}</strong>
          <p class="cross-mobile-route">${exchangeAEscaped} vs ${exchangeBEscaped}</p>
        </div>
        ${reliabilityChip}
      </div>
      ...
      <div class="cross-mobile-chips">
        <span class="chip cross-mobile-direction-chip">${directionEscaped}</span>
        <span class="chip lifecycle-chip lifecycle-${lifecycleEscaped}">${escapeHtml(translateLifecycle(item.lifecycle))}</span>
        <span class="chip regime-chip regime-${regimeEscaped}">${regimeLabelEscaped}</span>
      </div>
...
// OPTIMIZATION: Pre-compute exchange trade URLs for hot path
const _exchangeUrlCache = {};

function _getExchangeUrl(exchange, pair) {
  const key = `${exchange}:${pair}`;
  if (!_exchangeUrlCache[key]) {
    _exchangeUrlCache[key] = buildExchangeTradeUrl(exchange, pair);
  }
  return _exchangeUrlCache[key];
}

function renderCrossCell(column, item, tradeAlertModel) {
  const key = column.key;
  if (key === 'exchangeA') {
    const url = _getExchangeUrl(item.exchangeA, item.pairA);
    const link = url
      ? `<a class="exchange-link" href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer" title="${escapeHtml(item.exchangeA)}">${escapeHtml(item.exchangeA)}</a>`
      : escapeHtml(item.exchangeA);
    return buildCell(column.label, link);
  }
  if (key === 'pairA') {
    const pinIcon = state.pinnedIds.has(item.id) ? '📌' : '';
    const banIcon = state.bannedTokens.has(String(item.pairA || '').toUpperCase()) ? '🚫' : '';
    const pinBtn = `<button class="pin-btn" type="button" data-pin-id="${escapeHtml(item.id)}" title="Pin/unpin spread">${pinIcon || '📌'}</button>`;
    const banBtn = `<button class="ban-btn" type="button" data-ban-token="${escapeHtml(item.pairA)}" title="Ban/unban token">${banIcon || '🚫'}</button>`;
    return buildCell(column.label, `<span class="chip">${escapeHtml(item.pairA)}</span> ${pinBtn} ${banBtn}`);
  }
  if (key === 'exchangeB') {
    const url = buildExchangeTradeUrl(item.exchangeB, item.pairA);
    const link = url
      ? `<a class="exchange-link" href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer" title="${escapeHtml(item.exchangeB)}">${escapeHtml(item.exchangeB)}</a>`
      : escapeHtml(item.exchangeB);
    return buildCell(column.label, link);
  }
  if (key === 'direction') {
  ...
  if (key === 'holdingHoursRecommended') {
    return buildCell(
      column.label,
  ...
  if (key === 'expectedFundingCarryPercent') {
...
function renderFundingTable() {
  ...
  dom.fundingTable.querySelector('tbody').innerHTML = `${renderVirtualSpacer(headers.length, isDesktopVirtualized ? virtualWindow.beforeHeight : 0)}${virtualWindow.rows
    .map(
      (item) => {
        const urlA = buildExchangeTradeUrl(item.exchangeA, item.pair);
        const urlB = buildExchangeTradeUrl(item.exchangeB, item.pair);
        const exA = urlA ? `<a class="exchange-link" href="${escapeHtml(urlA)}" target="_blank" rel="noopener noreferrer">${escapeHtml(item.exchangeA)}</a>` : escapeHtml(item.exchangeA);
        const exB = urlB ? `<a class="exchange-link" href="${escapeHtml(urlB)}" target="_blank" rel="noopener noreferrer">${escapeHtml(item.exchangeB)}</a>` : escapeHtml(item.exchangeB);
        const countdownA = buildFundingCountdownHtml(item.nextFundingTimeA, item.fundingIntervalHoursA);
        const countdownB = buildFundingCountdownHtml(item.nextFundingTimeB, item.fundingIntervalHoursB);
        return `
        <tr>
          ${buildCell(t('cross.label.exchangeA'), exA)}
          ${buildCell(t('cross.label.exchangeB'), exB)}
          ${buildCell(t('cross.label.pairA'), `<span class="chip">${escapeHtml(item.pair)}</span>`)}
          ${buildCell(t('cross.label.fundingA'), `${formatNumber(item.fundingA, 4)} ${countdownA}`, valueClass(item.fundingA))}
          ${buildCell(t('cross.label.fundingB'), `${formatNumber(item.fundingB, 4)} ${countdownB}`, valueClass(item.fundingB))}
...
function renderFundingMobileCard(item) {
  ...
  return `
    <article class="mobile-data-card card-enter">
...
function renderHeatmapPage() {
  ...
  dom.heatmapGrid.innerHTML = rows
    .map((item) => {
      ...
      return `
        <button class="heat-tile card-enter" type="button" data-detail-id="${escapeHtml(item.id)}" style="--heat-tone:${heat};">
...
function showToast(message, tone = 'positive', durationMs = 4000) {
  if (!dom.toastContainer) return;
  const icons = { positive: '✓', warning: '⚠', negative: '✕' };
  const toast = document.createElement('div');
  toast.className = `toast toast-${tone}`;
  toast.innerHTML = `
    <span class="toast-icon">${icons[tone] || 'i'}</span>
...
function renderSkeleton(count = 4, type = 'metric') {
  const items = [];
  for (let i = 0; i < count; i++) {
    if (type === 'metric') {
      items.push(`<div class="skeleton skeleton-metric"></div>`);
    } else if (type === 'chip') {
      items.push(`<span class="skeleton skeleton-chip"></span>`);
    } else {
...
function renderConnectionDot() {
  ...
  dot.id = 'connection-indicator';
  let statusClass = 'disconnected';
  let label = 'Disconnected';
  if (state.marketStreamConnected) {
    statusClass = 'connected';
    label = 'Live';
  } else if (state.marketStreamMode === 'connecting') {
    statusClass = 'connecting';
    label = 'Connecting';
  }
  dot.className = `connection-dot ${statusClass}`;
  dot.textContent = label;
  dot.title = `WebSocket: ${label}`;
  container.appendChild(dot);
}

// ── Data freshness badge ───────────────────────────────────────────────
function renderFreshnessBadge() {
  const existing = document.querySelector('#freshness-badge');
  if (existing) existing.remove();
  const container = document.querySelector('.hero-metric');
  if (!container) return;
  const lastUpdated = state.lastUpdated || state.lastRefreshAt;
  if (!lastUpdated) return;
  const ageMs = Date.now() - new Date(lastUpdated).getTime();
  const ageMin = ageMs / 60000;
  let tone = 'fresh';
  let label = 'Fresh';
  if (ageMin > 5) { tone = 'old'; label = 'Stale'; }
  else if (ageMin > 2) { tone = 'stale'; label = 'Aging'; }
  const badge = document.createElement('span');
    badge.id = 'freshness-badge';
  badge.className = `freshness-badge ${tone}`;
...
function getLiveExitEtaMinutes(item) {
  const exitEtaMinutes = numberOrNull(item.exitEtaMinutes);
  if (exitEtaMinutes == null || exitEtaMinutes <= 0) {
    return exitEtaMinutes;
  }
  ...
  return Math.max(0, Number((exitEtaMinutes - elapsedMinutes).toFixed(1)));
}
...
function renderAiAnalyticsPage() {
  ...
  dom.aiAnalyticsList.innerHTML = analyticsRows.map((item) => {
    return `
      <article class="ai-analytics-card card-enter ${item.whaleIndex >= 70 || item.liquidationRiskPct >= 18 ? 'is-alert' : ''}">
...
function renderGlossaryPage() {
  ...
  dom.glossaryList.innerHTML = entries.map((entry) => `
    <article class="detail-note-card glossary-card card-enter">
...
function renderHeroRiskSummary() {
  ...
  const cards = [
    ...
    },
    {
      ...
      value: Number(alertSummary.active_count || 0) > 0
        ? `${t('hero.alertsActive', { count: Number(alertSummary.active_count || 0) })}${Number(alertSummary.critical_count || 0) > 0 ? ` · ${Number(alertSummary.critical_count || 0)} critical` : ''}`
        : t('hero.alertsClear'),
